...
const { isYYYYMMDD, hasValidPropertiesForReserving } = require("../utils/validation");
...
function hasValidPropertiesForUpdating(req, res, next) {
  ...
  /*
    ...
    These 3 lines are included in the DB rows, however we do not want
...
/*
  function that verifies that the request contains a query parameter with either the name `date` which is properly formatted as YYYY-MM-DD, 
  or the name `mobile_number` which can be any number. This function will call the express error handler if neither 
  `date` nor `mobile_number` is a query parameter in the URL.
 */
...
/*
 Verifies that previous reservation status is not "finished" since we cannot
